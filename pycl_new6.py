...
while width > 512:
  ...
  out_img = cl.Image(p.context, mf.READ_WRITE | mf.USE_HOST_PTR, image_format, hostbuf=out_buffer)

  p.program.downsample(p.queue, out_buffer.shape, None, in_img, out_img)

  cl.enqueue_read_image(p.queue, out_img, (0,0), (width, width), out_buffer).wait()

  # cl.enqueue_copy(p.queue, out_buffer, out_img, origin=(width,0), region=(width,width))

  print out_buffer.shape, width
