...
# original image
cl_buffers.append(cl.Image(p.context, mf.READ_ONLY | mf.USE_HOST_PTR, image_format, hostbuf=img))
...
while width > 512:
  ...
  cl_buffers.append(cl.Image(p.context, mf.READ_WRITE, image_format, (out_buffer.shape)))

  out_images.append(out_buffer)

print 'CL Buffers created!'

max_zoomlevel = len(out_images)

#
# run downsampling
#
for zoomlevel, cl_buffer in enumerate(cl_buffers):
  if (zoomlevel == max_zoomlevel):
    break

  out_zoomlevel = zoomlevel + 1
  out_buffer = out_images[zoomlevel]
  p.program.downsample(p.queue, out_buffer.shape, None, cl_buffer, cl_buffers[out_zoomlevel])
  # store the output
  
  cl.enqueue_read_image(p.queue, cl_buffers[out_zoomlevel], (0,0), out_buffer.shape, out_buffer).wait()
  
  print out_buffer.shape
  cv2.imwrite('/tmp/pycl_tex_z'+str(out_zoomlevel)+'.jpg', out_buffer)
